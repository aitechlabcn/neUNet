...
from neUNet_utils.wavelet_downsampling import WaveletDownsampleD, WaveletDownsampleArray
from nnunetv2.inference.predict_from_raw_data import nnUNetPredictor
from neUNet_utils.predict_from_multiscales_data import nnUNetWaveletPredictor
import numpy as np
import torch
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 wavelets: bool = True, device: torch.device = torch.device('cuda')):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.wavelet_scales = self._get_wavelet_scales()
    ...
    def get_dataloaders(self):
        ...
        # outputs?
        deep_supervision_scales = self._get_deep_supervision_scales()
        wavelet_scales = self._get_wavelet_scales()
        rotation_for_DA, do_dummy_2d_data_aug, initial_patch_size, mirror_axes = \
            self.configure_rotation_dummyDA_mirroring_and_inital_patch_size()

        # training pipeline
        tr_transforms = self.get_training_transforms(
            patch_size, rotation_for_DA, deep_supervision_scales, wavelet_scales, mirror_axes, do_dummy_2d_data_aug,
    ...
    def get_training_transforms(patch_size: Union[np.ndarray, Tuple[int]],
                                ...
                                wavelet_scales: Union[List, Tuple],
                                ...
                                use_wavelets: bool = False) -> AbstractTransform:
        tr_transforms = []
        if do_dummy_2d_data_aug:
            ignore_axes = (0,)
        ...
        if deep_supervision_scales is not None:
            tr_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='target',
                                                              output_key='target'))
        if use_wavelets:
            tr_transforms.append(WaveletDownsampleD(wavelet_scales, input_key='data', output_key='data', mode=1))
    ...
    def get_validation_transforms(deep_supervision_scales: Union[List, Tuple],
        ...
        if deep_supervision_scales is not None:
            val_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='target',
                                                               output_key='target'))
        if use_wavelets:
            val_transforms.append(WaveletDownsampleD(wavelet_scales, input_key='data', output_key='data', mode=1))
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        if self.use_wavelets:
            predictor = nnUNetWaveletPredictor(tile_step_size=0.5, use_gaussian=True, use_mirroring=True,
                                               perform_everything_on_gpu=True, device=self.device, verbose=False,
                                               verbose_preprocessing=False, allow_tqdm=False)
            predictor.manual_initialization(self.network, self.plans_manager, self.configuration_manager, None,
                                            self.dataset_json, self.__class__.__name__,
                                            self.inference_allowed_mirroring_axes, wavelet_scales)
        else:
