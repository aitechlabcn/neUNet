...
class PixelShuffle3d(nn.Module):
    ...
    def forward(self, x):
        ...
        input_view = x.contiguous().view(batch_size, nOut, self.scale[0], self.scale[1], self.scale[2], in_depth, in_height,
                                         in_width)
...
class ConvWithPixelShuffle(nn.Module):
    def __init__(self, conv_op, in_channels, out_channels, scales):
        ...
        n = np.prod(scales)
        self.conv1 = conv_op(in_channels, in_channels, 5, 1, 2)
        self.tanh = nn.Tanh()
        if self.dims == 3:
            self.conv2 = conv_op(in_channels, out_channels * n, 3, 1, 1)
        elif self.dims == 2:
            self.conv2 = conv_op(in_channels, out_channels * n, 3, 1, 1)
    ...
    def __init__(self,
        ...
        for s in range(1, n_stages_encoder):
            ...
            convswithpixelshuffle.append(ConvWithPixelShuffle(
                conv_op=encoder.conv_op, in_channels=input_features_below, out_channels=input_features_skip, scales=stride_for_transpconv
            ))
    ...
    def compute_conv_feature_map_size(self, input_size):
        ...
        for s in range(len(self.stages)):
            ...
            if convert_conv_op_to_dim(self.encoder.conv_op) == 3:
                output += np.prod([self.encoder.output_channels[-(s+2)] *12/8, *skip_sizes[-(s+1)]], dtype=np.int64)
            else:
                output += np.prod([self.encoder.output_channels[-(s + 2)] * 8/8, *skip_sizes[-(s + 1)]],dtype=np.int64)
            # segmentation
...
class SRUNet(PlainConvUNet):
    def __init__(self,
        ...
        self.decoder = SRUnetDecoder(self.encoder, num_classes, n_conv_per_stage_decoder, deep_supervision,
                                     nonlin_first=nonlin_first)
