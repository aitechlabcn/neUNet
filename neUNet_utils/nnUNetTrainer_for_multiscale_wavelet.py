...
from SRUNet.wavalet_downsampling import WaveletDownsampleD, WaveletDownsampleArray
from SRUNet.predict_from_multiscales_data import nnUNetPredictor
import numpy as np
import torch
...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # self.wavalet_scales = self._get_wavalet_scales()
            # waveletdownsample_array = WaveletDownsampleArray(ds_scales=self.wavalet_scales)
            # img = torch.zeros(([1,self.num_input_channels] + self.configuration_manager.patch_size))
            # img = waveletdownsample_array(img)
            # if isinstance(img, list):
            #     img = [i.to(self.device, non_blocking=True) for i in img]
            # else:
            #     img = img.to(self.device, non_blocking=True)
            # flops, params = profile(self.network,inputs=(img,))
            # self.print_to_log_file('FLOPs:', flops, 'Params:', params)
    ...
    def _get_wavalet_scales(self):
        wavalet_scales = [[1.0, 1.0, 1.0]]
        wavalet_scales.extend(list(list(i) for i in 1 / np.cumprod(np.vstack(
            self.configuration_manager.pool_op_kernel_sizes[:5]), axis=0))[:-1])
        return wavalet_scales
    ...
    def get_dataloaders(self):
        ...
        # outputs?
        deep_supervision_scales = self._get_deep_supervision_scales()
        wavalet_scales = self._get_wavalet_scales()
        rotation_for_DA, do_dummy_2d_data_aug, initial_patch_size, mirror_axes = \
            self.configure_rotation_dummyDA_mirroring_and_inital_patch_size()

        # training pipeline
        tr_transforms = self.get_training_transforms(
            patch_size, rotation_for_DA, deep_supervision_scales, wavalet_scales, mirror_axes, do_dummy_2d_data_aug,
    ...
    def get_training_transforms(patch_size: Union[np.ndarray, Tuple[int]],
                                ...
                                wavalet_scales: Union[List, Tuple],
        ...
        if deep_supervision_scales is not None:
            tr_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='target',
                                                              output_key='target'))
        tr_transforms.append(WaveletDownsampleD(wavalet_scales, input_key='data', output_key='data', mode=1))
    ...
    def get_validation_transforms(deep_supervision_scales: Union[List, Tuple],
        ...
        if deep_supervision_scales is not None:
            val_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='target',
                                                               output_key='target'))
        val_transforms.append(WaveletDownsampleD(wavalet_scales, input_key='data', output_key='data', mode=1))
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if not self.label_manager.has_regions:
            ...
            tp_hard = tp_hard[1:]
            fp_hard = fp_hard[1:]
            fn_hard = fn_hard[1:]  # 去除背景类、求batch总和
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        self.logger.log('mean_fg_dice', mean_fg_dice, self.current_epoch)  # 总平均dice（除背景）
        self.logger.log('dice_per_class_or_region', global_dc_per_class, self.current_epoch)  # 每类平均dice
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        predictor.manual_initialization(self.network, self.plans_manager, self.configuration_manager, None,
                                        ...
                                        self.inference_allowed_mirroring_axes, wavalet_scales)
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for k in dataset_val.keys():
                ...
                data, seg, properties = dataset_val.load_case(k)  # 预处理后的图片、标签
